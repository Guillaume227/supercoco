        ...
        if not collision:
            ...
            for Index in OrdreIndex:
                ...
                if collision and obstacles:
                    ...
                    Chocs = ( obstacle.collisione( mouv, self.rect ) for obstacle in obstacles if obstacle._penetrable and self.rect.colliderect(obstacle.rect) )
                    ...
                    for side, chocs in ChocsBySide.iteritems():
                    
                        if side in ( BAS, HAUT ):
                            chocs = sorted(chocs, cmp = TriChocsFunc(self.rect.centerx))
                        ...
                        for _dist, penetrable, obj in chocs:
                            ...
                            if not penetrable and not self._penetrable[side]:
                                ...
                                if ( side == HAUT or side == BAS ) and isinstance( self, Personnage ) and self.taille > 0 and len(chocs) == 1:
                                ...
                                if side == HAUT:
                                    ...
                                    if obj.rebord_ and getattr(self, 'marche_bas_', False):
                                        ...
                                        if deplacement[0] != 0:
                                            ...
                                            if not any( [ not Elem.penetrable_haut_ for Elem in self._crible.Intersecte(rectPas, self) ] ):
                                    ...
                                    if getattr( self, 'marche_haut_', False ) and not isinstance(obj, EnemiInterface):
                                        ...
                                        if not any( [ not Elem.penetrable_haut_ or not Elem.penetrable_traverse_ for Elem in self._crible.Intersecte(rectPas, self) ] ):
                                            # Monte la marche
                                            self.rect.bottom = obj.rect.top
                                            self.deplace()
                                            return
    ...
    def casser(self, cote=None):
        ...
        for i in 0,1:
            for j in 0,1:
                ...
                Morceau( origTopLeft + (imgL*i, imgH*j), images = [ sousImg ], vX = 2 if i else -2, impulsion = -12 if j else -18 )
        ...
        if self._nom == 'coco':
            ...
            for nomCouleur in NomCouleurs:
                ...
                for racine in 'cocogd', 'cocogdfire', 'cococoudaslip', 'cococouda':
                    ...
                    nomImagesGrand += [ racine +"nage%d.png"%i for i in xrange(1,7) ]
            ...
            for taille in 0,1:
                ...
                for coul_num, couleur in enumerate(['', 'rouge', 'blanc', 'noir']) :
                    ...
                    nomImages  = [ nomCouleur+"%d.png"%i for i in SequenceImage ]
                    nomImages += [ nomCouleur+"nage%d.png"%i for i in xrange(1,7) ]
...
class Panneau( Dessinable ):
    ...
    def maj(self):
        ...
        if self.compteur <= self.vitesse_defilement_:
            ...
            pas = self.cadre[1] / float(self.vitesse_defilement_)
    ...
    def EffetJoueur( self, joueur, side ):
        ...
        if not self._perso:
            ...
            if self.monde_suivant_ and side == cote_entree and ( \
                (side in (HAUT,     BAS) and abs( joueur.rect.centerx - self.rect.centerx ) < self.rayon_absorbtion_ ) \
             or (side in (DROITE, GAUCHE) and abs( joueur.rect.bottom - self.rect.bottom) < self.rayon_absorbtion_ ) ) \
...
class PortailMonde( Dessinable, Passage ):
    ...
    def EffetJoueur(self, joueur, side):
        if self.monde_suivant_ and abs(joueur.rect.centerx - self.rect.centerx) <= 2 and abs(joueur.rect.bottom - self.rect.bottom) <= 2:
            joueur.efface()
            self.active = True
        ...
        if self.declenche_fin_niveau_:
            ...
            if joueur.rect.bottom < self.rect.bottom - 10:
                ...
                if joueur.Controle.Bas:
                    ...
                    points = int(round(5000 * self.pos_drapeau_/float(maxPosDrapeau),-2)) # arrondi a la centaine
                    if points > 0:
...
Elements = dict( Surprises  = [ ChampiTaille, ChampiVert, Burger, Choppe, Piece, PieceBleue, FleurDeFeu, Bague, Etoile, PersoNonJoueur, 
                 ...
                 CapFerret  = [ Dune,Sables, EcureuilNuageux, Calmar,Crabe, Oisson, OissonGris, Vaguelettes, PleineEau, PeuDEau, PinCime, 
                 ...
                 Mechants   = [ Slub, Tortue, TortueRouge, TortueAiles, TortueRougeAiles, FleurCarnivore,
                                Canon, CanonPetit, CanonGrand, Calmar, Crabe, EcureuilNuageux, Herisson, Oisson, OissonGris, Pigeon, RatNoir],
                 
                 Accessoires= [ Tuyeau, TuyeauCourt, TuyeauGrand, TuyeauMini, Tuyeau_Jointure, Tuyeau_Tube, Tuyeau_Bouche, 
                                Poutrelle, PoutrelleLongue, Tremplin, MatDrapeau, PortailMonde, Coeur, Panneau, PanneauLarge, Generateur, Ecriteau ],
