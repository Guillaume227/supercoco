...
class Partie(object):
    ...
    def Intro_Niveau(self,affiche=True):
        ...
        if affiche:
            ...
            ren = self.police.render(langues.Traduc(langues.Monde)+ " " + self.niveau.nom.split("=")[0], 1, (255, 255, 255))
    ...
    def Affiche_Stats(self):
        ...
        self.affiche_texte("%05d" % (self.points+self.perso.points),pos=(-EcranL+MargeCote+texteDim[0], MargeH+PoliceH),centre=(False,False))
        
        # Nombre de pieces 
        self.affiche_texte("x%02d" % self.perso.boursePieces,pos=(MargeCote+100, MargeH+PoliceH),centre=(False,False))
        
        ecran.blit( self.mini_pieces[int(self.temps_discret/9)%len(self.mini_pieces)], (MargeCote+86, MargeH+PoliceH-3) )
        ...
        self.affiche_texte("%d" % max(0,self.compte_a_rebours),pos=(-MargeCote, MargeH+PoliceH),centre=(False,False))
